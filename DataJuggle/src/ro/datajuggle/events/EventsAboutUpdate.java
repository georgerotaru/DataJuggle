 ...
 * @author admin
 */
public class EventsAboutUpdate {
...
private String eventName = null;
    ...
    public EventsAboutUpdate() throws SQLException, ClassNotFoundException, NullPointerException, SQLDataException {
        try {
            ...
            for (int c=0; c<eventsList.size(); c++) {
                int counter = 0;
                eventId = eventsList.get(c).toString();
                ...
                try {
                    statement.execute("UPDATE APP.EVENTS_ABOUT SET NAME= '"+eventName+"', CITY='"+eventCity+"', "
                            + "PLACE='"+eventPlace+"', ATTENDING="+eventAttending+", INTERESTED="+eventInterested+", LATITUDE="+eventLatitude+", LONGITUDE="+eventLongitude+", "
                            + "START_DATE='"+eventStartDate+"', START_TIME='"+eventStartTime+"', END_DATE='"+eventEndDate+"', END_TIME="
                            + "'"+eventEndTime+"' WHERE ID="+"'"+eventId+"'");
